...
import { MapPin, Calendar, Users, DollarSign, Edit, Trash2, ArrowLeft, Sparkles, Plus, Receipt, Download, MessageCircle } from 'lucide-react';
...
const TripDetail = () => {
  ...
  const fetchTripData = async () => {
    try {
      ...
      if (tripData.ai_generated_plan) {
        try {
          const plan = typeof tripData.ai_generated_plan === 'string' ? JSON.parse(tripData.ai_generated_plan) : tripData.ai_generated_plan;
          setParsedPlan(plan);
        } catch (parseError) {
      ...
      const {
        data: expenseData,
        error: expenseError
      } = await supabase.from('expenses').select('*').eq('trip_id', id).order('date', {
        ascending: false
      });
  ...
  const generateAIPlan = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('generate-trip-plan', {
        body: {
          trip: {
  ...
  const handlePlanUpdate = (updatedPlan: any) => {
    ...
    setTrip(prev => prev ? {
      ...prev,
      ai_generated_plan: JSON.stringify(updatedPlan)
    } : null);
  };
  ...
  return <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex space-x-2">
          {parsedPlan}
          {trip?.status === 'planned' && <Button variant="outline" size="sm" onClick={markAsActive}>
              Start Trip
            </Button>}
          {trip?.status === 'active' && <Button variant="outline" size="sm" onClick={markAsCompleted}>
              Mark Complete
            </Button>}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex space-x-2">
                {parsedPlan && <>
                    <Button onClick={() => setShowChat(!showChat)} size="sm" variant="outline">
                ...
                <Button onClick={generateAIPlan} disabled={generatingPlan} size="sm">
                  {generatingPlan ? <>
            ...
            <CardContent>
              {parsedPlan ? <>
                  {showChat && <div className="mb-6">
                      <TripPlanChat tripId={id!} currentPlan={parsedPlan} onPlanUpdate={handlePlanUpdate} />
                    </div>}
                  <TripPlanDisplay plan={parsedPlan} expenses={expenses} tripDetails={{
                title: trip.title,
                destination: trip.destination,
                start_date: trip.start_date,
                end_date: trip.end_date,
          ...
          <Card>
            ...
            <CardContent>
              ...
              {expenses.length > 0 ? <div className="space-y-2">
                  {expenses.slice(0, 5).map(expense => <div key={expense.id} className="flex justify-between items-center py-2 border-b last:border-b-0">
                      <div>
                  ...
                  {expenses.length > 5 && <Link to={`/trip/${id}/expenses`} className="block text-center text-sm text-purple-600 hover:text-purple-700 py-2">
